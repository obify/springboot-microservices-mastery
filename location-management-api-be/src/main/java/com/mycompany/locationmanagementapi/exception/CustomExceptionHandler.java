...
public class CustomExceptionHandler {
    ...
    public ResponseEntity<List<ErrorModel>> handleFieldConstraintException(
            ConstraintViolationException cve, WebRequest webRequest){

        List<ErrorModel> errorModelList =  new ArrayList<>();

        Set<ConstraintViolation<?>> fieldErrors = cve.getConstraintViolations();

        for(ConstraintViolation fe :fieldErrors) {
            ErrorModel errorModel = new ErrorModel();
            errorModel.setCode(fe.getPropertyPath().toString());
            errorModel.setMessage(fe.getMessage());
            logger.debug("Inside handleFieldValidationException: {}, {}", errorModel.getCode(), errorModel.getMessage());
            errorModelList.add(errorModel);
        }
